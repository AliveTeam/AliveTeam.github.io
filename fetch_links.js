...
const fetchJson = (url) => fetch("https://api.github.com/repos/" + url).then(r => r.json())
const urls = {
  stable: "AliveTeam/alive_reversing/releases/latest",
  beta: "mouzedrift/alive_reversing/commits/abi_break_refcount",
...
function setDate(json, accessor, selector) {
  ...
  const formatted = new Intl.DateTimeFormat("en-UK", { year: "numeric", month: "short", day: "numeric" }).format(date);
...
false &&
  window.addEventListener("load", async () => {
    ...
    Array.from(document.querySelectorAll(".artifact")).map(link => replaceURL(link, assets))
